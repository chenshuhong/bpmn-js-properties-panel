...
var textField = function(options, defaultParameters) {
  ...
  resource.html =
		...
		'</button>' +
		'</div>'+
	  '<div id="camunda-' + resource.id + '-select" class="camunda-usertask"' +
	  ' /></div>' +
	...
	resource.setControlValue = function(element, entryNode, inputNode, inputName, newValue) {
		...
		if (value){
			...
			selectedValue&&selectedValue.map(user=>{
				userNameArray.push(user.userName||user.name)
			})
			entryNode.querySelector('#camunda-' + options.id + '-select').innerText = userNameArray.join()
		}else {
			entryNode.querySelector('#camunda-' + options.id + '-select').innerText = ''
		}
